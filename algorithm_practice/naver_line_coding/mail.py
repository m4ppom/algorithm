...
def solution(record):
    ...
    while record:
        ...
        if len(a) ==2:
            ...
            order = a[0]
            info = False
        if order == 'DELETE' and temp :
            temp.pop()
            continue
        elif order == 'DELETE' and temp == False:
            continue
        elif order == 'SAVE' and temp:
            while temp:
                save.append(temp.pop(0))
            continue
        elif order == 'SAVE' and temp == False:
            continue
        elif order == 'RECEIVE':
            temp.append(info)
            continue
